...
def training_instances():
    return [f"p-{keys_locks}-{keys_locks}-{prob_key_in_goal}-{shapes}-{x}-{y}-{seed}" for keys_locks in ["1", "2"] for prob_key_in_goal in [100] for shapes in [1] for x in [2] for y in [2] for seed in range(0, 100)] + \
           [f"p-{keys_locks}-{keys_locks}-{prob_key_in_goal}-{shapes}-{x}-{y}-{seed}" for keys_locks in ["20", "02", "11"] for prob_key_in_goal in [100] for shapes in [2] for x in [2] for y in [2] for seed in range(0, 100)]
